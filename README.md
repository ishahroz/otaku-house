...
2. **SSH into Django EC2**:
    ...
    eval "$(ssh-agent -s)"  # to start an SSH agent
    ...
    ssh -A -i otaku-house-django-ec2.pem ec2-user@10.0.0.200
    ```
...
- **Database Engine**: MariaDB (due to restrictions on other DB engines).
- **Instance Specifications**:
    - Name: `otaku-house-rds`.
    - Username: `ishahroz`.
    - Password: `ishahrozTEST123.,`.
...
- **Inbound Rules**:
    ...
    - Protocol: TCP.
    - Port Range: 3306.
    - Source: Self (allow requests from the same SG).

### Part 7: Test RDS Connection

- **SSH to Bastion Host and Django EC2**:
    - Install MariaDB: `sudo dnf install mariadb105`.
    - Connect to RDS:
    ```bash
...
- **Create Load Balancer**:
    ...
    - Security Group: `otaku-house-alb`.
...
- **Configure Listeners**:
    ...
    - Set the protocol to TCP and the port range to 8000.
